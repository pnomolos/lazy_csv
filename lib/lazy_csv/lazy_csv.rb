...
class LazyCSV
  ...
  def initialize(input, options)
    ...
    @file_line_count = 0
    @csv_line_count = 0
    @input_is_io = input.respond_to?(:readline)
  ...
  def initialize_io
    ...
    if @options[:row_sep] == :auto
      @options[:row_sep] = LazyCSV.guess_line_ending(@io, @options)
      @io.rewind
    end
    ...
    if @options[:headers_in_file] # extract the header line
      ...
      raise LazyCSV::IncorrectOption, 'ERROR [lazy_csv]: If :headers_in_file is set to false, you have to provide :user_provided_headers or :parse_to_arrays'
    end
    ...
    if @options[:user_provided_headers] && @options[:user_provided_headers].class == Array && !@options[:user_provided_headers].empty?
      ...
      if file_header_size && headerA.size != file_header_size
        raise LazyCSV::HeaderSizeMismatch, "ERROR [lazy_csv]: :user_provided_headers defines #{headerA.size} headers != CSV-file has #{file_header_size} headers"
      end
    else
    ...
    if @options[:parse_to_arrays]
      ...
      if @options[:convert_values_to_numeric]
        hash.each_with_index do |v, k|
          ...
          next if LazyCSV.only_or_except_limit_execution(@options, :convert_values_to_numeric, k)
